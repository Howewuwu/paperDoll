//
//  GirlDressUpViewController.swift
//  paperDoll
//
...
class GirlDressUpViewController: UIViewController {
    ...
    @IBAction func itemCategoryButtonCheck(_ sender: UIButton) {
        ...
        case 62 :
            for i in 0...4{
                itemButtons[i].configuration?.image = UIImage(named: "GirlClothes-"+"\(i+1)")
            }
            index = 62
        case 63 :
            for i in 0...2{
                itemButtons[i].configuration?.image = UIImage(named: "GirlPants-"+"\(i+1)")
            }
            for e in 3...4{
                itemButtons[e].configuration?.image = UIImage(systemName: "nosign")
            }
        ...
        case 64 :
            for i in 0...2{
                itemButtons[i].configuration?.image = UIImage(named: "GirlShoes-"+"\(i+1)")
            }
            index = 64
        case 65 :
            itemButtons[0].configuration?.image = UIImage(named: "GirlSunglass-1")
            for e in 1...4{
                itemButtons[e].configuration?.image = UIImage(systemName: "nosign")
            }
            index = 65
    ...
    @IBAction func dressUpOnGirlButton(_ sender: UIButton) {
        ...
        case 62 :
            clothesImageView.image = UIImage(named: "GirlClothes-"+"\(sender.tag + 1)")
        case 63 :
            pantsImageView.image = UIImage(named: "GirlPants-"+"\(sender.tag + 1)")
        case 64 :
            shoesImageView.image = UIImage(named: "GirlShoes-"+"\(sender.tag + 1)")
        case 65 :
            sunglassImageView.image = UIImage(named: "GirlSunglass-"+"\(sender.tag + 1)")
        default :
            print("oh")
        }
    }
    ...
    func initialScreen () {
        ...
        pantsImageView.image = UIImage(named: "GirlPants-1")
